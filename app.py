...
def markdown_to_html(filepath):
    with open(filepath, 'r', encoding='utf-8') as f:
        text_file = f.read()
    html = markdown.markdown(text_file,
...
def get_arcticles_md(filepath):
    if functions_working_with_fs.check_file_to_read(filepath):
        return functions_working_with_fs.read_json_file(filepath)
...
def convert_article(article, topic, file_md, file_html, link_to_main):
    article_html = markdown_to_html(file_md)
    ...
    functions_working_with_fs.save_html_to_file(html_page, file_html)
...
def delete_old_site_page():
    if functions_working_with_fs.check_dir_to_write(
            os.path.join(CONFIG.path_to_html, CONFIG.path_to_markdown)):
...
if __name__ == '__main__':
    ...
    for article in articles_md['articles']:
        ...
        if not functions_working_with_fs.check_file_to_read(file_md):
